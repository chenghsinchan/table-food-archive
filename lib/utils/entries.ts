...
export function searchEntries(entries: FoodEntry[], query: string) {
  ...
  return entries.filter((entry) => {
    const haystack = [
      ...
      entry.daypart,
      entry.mood ?? "",
      entry.atmosphere && entry.atmosphere.x < 40 ? "tired drained low energy" : "",
      entry.atmosphere && entry.atmosphere.x > 60 ? "energised energetic" : "",
      entry.atmosphere && entry.atmosphere.y > 60 ? "quiet soothing restorative calm" : "",
      entry.atmosphere && entry.atmosphere.y < 40 ? "vivid lively stimulating" : "",
